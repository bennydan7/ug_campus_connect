        ...
        <div className="space-y-2 p-4">
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {displayed.map((a, i) => (
                  <div key={i} className="relative h-8 w-8 flex-shrink-0">
                    {a.avatarUrl ? (
                      <img src={a.avatarUrl} alt={`Avatar of ${a.name}`} className="h-8 w-8 rounded-full border-2 border-white object-cover" />
                    ) : (
              ...
              <div className="text-sm text-muted-foreground">{event.attendees ? `${event.attendees} attending` : '—'}</div>
            </div>
            <button className="rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow hover:brightness-95">RSVP</button>
          </div>
        </div>
      </article>
    </Link>
