...
export type Category = 'All' | 'Academic' | 'Social' | 'Sports' | 'Career' | 'Arts' | 'Health'
const categories: Category[] = ['All', 'Academic', 'Social', 'Sports', 'Career', 'Arts', 'Health']

export default function FilterBar({ active, onChange }: { active: Category; onChange: (c: Category) => void }) {
  return (
    <div className="sticky top-[68px] z-30 border-b bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto flex items-center justify-between gap-3 py-3 px-4">
          ...
          {categories.map((c) => (
            <button
              ...
              className={
                ...
                (c === active ? 'bg-primary text-primary-foreground border-transparent shadow' : 'bg-white hover:bg-muted/60 text-foreground')
              }
            >
              {c}
            </button>
        ...
        <div className="hidden items-center gap-3 sm:flex">
          <button onClick={() => onChange('All')} className="text-sm text-primary underline-offset-4 hover:underline">Clear filters</button>
